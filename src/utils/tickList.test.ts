...
describe('TickList', () => {
  ...
  describe('#nextInitializedTick', () => {
    //lte = true -> always refer to the lower closest tick in tick list (result <= tick)
    //lte = false mean gte -> always refer to the higher closest tick in tick list (result > tick)
    let ticks: Tick[]
  ...
  describe('#nextInitializedTickWithinOneWord', () => {
    //same, but the step space is 2^8
    //for lte 2^8
    //for !lte 2^8-1
    let ticks: Tick[]
